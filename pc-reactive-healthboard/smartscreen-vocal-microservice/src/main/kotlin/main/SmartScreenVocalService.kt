...
import main.Topics.SMART_SCREEN_VOICE_TOPIC
import java.util.HashMap


object Topics {
    const val SMART_SCREEN_VOICE_TOPIC = "SMART_SCREEN_VOICE_TOPIC"
}

class SmartScreenVocalService: AbstractVerticle() {
    ...
    private fun createServer() {
        ...
        router.route("/smart-screen-events/*").handler(eventBusHandler())
    ...
    private fun apiRouter(): Router {
        ...
        router.get("/").handler { req -> req.response().end("Welcome to the Reactive Smart Screen!")}
        router.post("/visualizzaInformazioni").handler(this::handleShowInfo)

        return router
    }

    private fun handleShowInfo(routingContext: RoutingContext) {
        vertx.executeBlocking<Unit>({ future ->

            val smartScreenVocalTable = dynamoDb.getTable("SmartScreenVocalStorage")

            val json = routingContext.bodyAsJson
                    .put("tipoComando", "visualizzaInformazioni")
            ...
            smartScreenVocalTable.putItem(item)
    ...
    private fun publishEvent(roomId: String, json: JsonObject) {
        vertx.eventBus().publish(SMART_SCREEN_VOICE_TOPIC, json)
